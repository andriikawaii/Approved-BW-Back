...
$specFiles = [
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\branford-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\darien-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\fairfield-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\greenwich-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\guilford-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\hamden-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\madison-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\milford-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\new-canaan-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\new-haven-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\norwalk-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\orange-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\ridgefield-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\stamford-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\westport-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\service-town-pages\\basement\\woodbridge-ct.md',
];

$templateRegistry = config('page-template-sections', []);

function normalizeNewlinesBasementTown(string $text): string
{
    return str_replace(["\r\n", "\r"], "\n", $text);
}

function normalizeUrlPathBasementTown(string $url): string
{
    $url = trim($url);
...
function isDecorativeLineBasementTown(string $line): bool
{
    $trimmed = trim($line);
    if ($trimmed === '') {
        return false;
...
function extractValueAfterLabelBasementTown(string $text, string $labelPattern): ?string
{
...
function findSectionSegmentBasementTown(string $text, array $startPatterns, array $endPatterns): string
{
    $startPos = null;
    $startLen = 0;
...
function parseContentBlocksBasementTown(string $contentSegment): array
{
    $lines = explode("\n", $contentSegment);
    $h1 = null;
    $pendingH1Line = false;
    ...
    foreach ($lines as $line) {
        ...
        if (isDecorativeLineBasementTown($line)) {
            continue;
        }
...
function parseCtaBasementTown(string $ctaSegment): array
{
    $lines = explode("\n", $ctaSegment);
    $clean = [];
...
function parseSpecBasementTown(string $filePath): array
{
    if (!is_file($filePath)) {
    ...
    $text = normalizeNewlinesBasementTown($raw);
    $text = preg_replace('/^\xEF\xBB\xBF/u', '', $text);

    $url = null;
    if (preg_match('/^URL:\s*(.+)$/mi', $text, $m)) {
        $url = normalizeUrlPathBasementTown($m[1]);
    }
    ...
    $seoTitle = extractValueAfterLabelBasementTown($text, '/^Meta Title[^\n]*:\s*$/mi');
    ...
    $seoDescription = extractValueAfterLabelBasementTown($text, '/^Meta Description[^\n]*:\s*$/mi');
    ...
    $contentSegment = findSectionSegmentBasementTown(
        $text,
    ...
    $content = parseContentBlocksBasementTown($contentSegment);
    if (!$content['h1']) {
    ...
    $ctaSegment = findSectionSegmentBasementTown(
        $text,
        ['/^CTA\s*$/mi', '/^FINAL CTA\s*$/mi', '/^\[CTA\]\s*$/mi'],
    ...
    $cta = parseCtaBasementTown($ctaSegment);
...
function chooseSectionTypeBasementTown(array $allowed, array $candidates): ?string
{
...
foreach ($specFiles as $filePath) {
    ...
    $spec = parseSpecBasementTown($filePath);
    ...
    $heroType = chooseSectionTypeBasementTown($allowed, ['hero_service_location', 'hero', 'service_hero', 'page_hero']);
    if (!$heroType) {
    ...
    $ctaType = chooseSectionTypeBasementTown($allowed, ['cta_block', 'cta_dark_band']);
    if (!$ctaType) {
