...
$specFiles = [
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\town-hub-pages\\greenwich-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\town-hub-pages\\madison-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\town-hub-pages\\new-haven-ct.md',
    'C:\\Users\\georg\\Downloads\\builtwell-ct-content-handoff\\content\\town-hub-pages\\westport-ct.md',
];

$templateRegistry = config('page-template-sections', []);

function normalizeNewlinesTown(string $text): string
{
    return str_replace(["\r\n", "\r"], "\n", $text);
}

function normalizeUrlPathTown(string $url): string
{
    $url = trim($url);
...
function isDecorativeLineTown(string $line): bool
{
    $trimmed = trim($line);
    if ($trimmed === '') {
        return false;
...
function extractValueAfterLabelTown(string $text, string $labelPattern): ?string
{
...
function findSectionSegmentTown(string $text, array $startPatterns, array $endPatterns): string
{
    $startPos = null;
    $startLen = 0;
...
function parseContentBlocksTown(string $contentSegment): array
{
    $lines = explode("\n", $contentSegment);
    $filtered = [];
    $h1 = null;
...
function parseCtaTown(string $ctaSegment): array
{
    $lines = explode("\n", $ctaSegment);
    $clean = [];
    ...
    foreach ($clean as $line) {
        ...
        if ($line === '[CTA Block]' || strcasecmp($line, 'CTA') === 0 || strcasecmp($line, 'FINAL CTA') === 0) {
            continue;
        }
...
function parseSpecTown(string $filePath): array
{
    if (!is_file($filePath)) {
    ...
    $seoTitle = extractValueAfterLabelTown($text, '/^Meta Title[^\n]*:\s*$/mi');
    if (!$seoTitle) {
        throw new RuntimeException("Meta Title not found in spec: {$filePath}");
    }

    $seoDescription = extractValueAfterLabelTown($text, '/^Meta Description[^\n]*:\s*$/mi');
    if (!$seoDescription) {
        throw new RuntimeException("Meta Description not found in spec: {$filePath}");
    }

    $contentSegment = findSectionSegmentTown(
        $text,
    ...
    $content = parseContentBlocksTown($contentSegment);
    if (!$content['h1']) {
    ...
    $ctaSegment = findSectionSegmentTown(
        $text,
        ['/^CTA\s*$/mi', '/^FINAL CTA\s*$/mi', '/^\[CTA\]\s*$/mi'],
        ['/^IMAGE RECOMMENDATIONS\s*$/mi', '/^\[IMAGE RECOMMENDATIONS\]\s*$/mi', '/^INTERNAL LINKS\s*$/mi', '/^\[INTERNAL LINKS\]\s*$/mi', '/^FOOTER\s*$/mi']
    );
    if ($ctaSegment === '') {
        throw new RuntimeException("CTA section not found in spec: {$filePath}");
    }

    $cta = parseCtaTown($ctaSegment);
...
function chooseSectionTypeTown(array $allowed, array $candidates): ?string
{
...
foreach ($specFiles as $filePath) {
    ...
    $heroType = chooseSectionTypeTown($allowed, ['hero', 'service_hero', 'page_hero', 'hero_service_location']);
    if (!$heroType) {
    ...
    $ctaType = chooseSectionTypeTown($allowed, ['cta_block', 'cta_dark_band']);
    if (!$ctaType) {
    ...
    $ctaSubtitle = null;
    $ctaExtra = $spec['cta']['extra'];
    if (count($ctaExtra) > 2) {
        $ctaSubtitle = implode(' | ', array_slice($ctaExtra, 2));
    }
